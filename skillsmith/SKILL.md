---
name: skillsmith
description: "Manage, sync, upgrade, and create Claude skills across the claude-skills repo and the on-device copies (~/.claude/skills global + per-project .claude/skills). Modes — sync: reconcile repo and device by comparing each skill's metadata.version, install repo-ahead skills onto the device, surface device-ahead / unversioned / diverged copies, honoring fork + external exceptions (cleanup stub, npx-managed impeccable). upgrade: bump a skill's version, regenerate the VERSIONS.md registry, run the Cowork-compliance check (description max 1024 chars, lean SKILL.md, detail in references/), add a changelog entry. create: author a new skill via the official skill-creator, then make it upgradeable (version, register, Cowork-validate, place in repo, optionally install). Triggers: 'sync my skills', 'upgrade a skill', 'bump a skill version', 'create a skill', 'install my skills', 'update the skill registry', 'are my skills cowork-installable', '/skillsmith'. Scripts in scripts/; procedures in references/."
metadata:
  version: 1.0.0
---

# skillsmith — manage, sync, upgrade & create Claude skills (v1.0.0)
...
This skill is versioned (`metadata.version` in frontmatter, currently **1.0.0**) and self-registers in `VERSIONS.md`. Bump it on every change — PATCH for fixes/wording, MINOR for a new mode or capability, MAJOR for a breaking workflow change — and add a §Changelog row.
