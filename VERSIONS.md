...
Canonical version + Cowork-compliance status of every skill. **Source of truth is the top-level
`version:` field** in each skill's `SKILL.md` frontmatter; this table mirrors it.
...
| **SKILL.md body** | **< 500 lines ideal** (soft — "go longer if needed", but add hierarchy + `references/` pointers as you approach it) | `skill-creator` authoring guidance |
| **SKILL.md body** | **keep well under ~10k words** for the Cowork installer (a 15,342-word file is known to fail install; references/ bundled files do NOT count) | empirical (`cleanup` §1.9.0) |

**Status:** `✓ lean` = comfortably within all · `○ large` = installable but over the 500-line ideal or
>5k words (trim candidate) · `⚠ body>10k` = near/over the empirical Cowork word ceiling — lean-pass
candidate · `✗ desc>1024` = hard violation, will reject.

**On any change:** bump the skill's `version:`, update its row here, keep the description ≤1024 chars,
and prefer pushing detail into `references/` over growing SKILL.md. See [`README.md`](README.md).

> `vendored`/`external` skills also carry an upstream `metadata.version` (provenance); the top-level
> `version:` is *our* tracked version and is what this registry records.
...
| [`cleanup`](cleanup/SKILL.md) | 1.14.1 | own | 130 / 973 | 3607 / 233 | ✓ lean | Multi-mode hygiene skill for Lex Council |
...
| [`conquering-campaign`](conquering-campaign/SKILL.md) | 3.8.2 | own | 140 / 1003 | 10290 / 467 | ⚠ body>10k | Multi-wave campaign skill for work too big for one pass |
...
| [`performance`](performance/SKILL.md) | 1.0.0 | vendored | 32 / 219 | 1034 / 355 | ✓ lean | Optimize web performance for faster loading and better user experience |
...
_28 skills — 22 lean · 5 large (installable, over the 500-line ideal) · 1 near the word ceiling · 0 hard violations. Last synced 2026-06-15._
