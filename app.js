...
// ════════════════════════════════════════════════════════════════════════
// The arsenal registry is the ONE source of truth (star-alliance-arsenal/models.json),
// emitted into guild-data as GUILD.models by build.py. Derive MODELS from it so the
// dashboard never hand-lists model facts. role 'media' -> 'doer' for the UI (pip + order).
const MODELS = Object.fromEntries(
  Object.entries((typeof GUILD !== "undefined" && GUILD.models) || {})
    .map(([id, m]) => [id, { ...m, role: m.role === "media" ? "doer" : m.role }])
);
...
const CLOUD_TAG = Object.fromEntries(
  Object.entries(MODELS).filter(([, m]) => m.cloud_tag).map(([id, m]) => [id, m.cloud_tag])
);
...
for (const [wid, st] of Object.entries((GUILD.meta && GUILD.meta.weaponStatus) || {})) { if ((st === "deactivated" || st === "dark") && !disabledWeapons.includes(wid)) disabledWeapons.push(wid); }
