#!/usr/bin/env python3
"""agent_level.py — the Quartermaster's agent-leveling console.

An agent's level is a CRAFT-DEPTH meter (arsenal + specialty), EARNED by an
...
level → log an agent-upgrade entry → rebuild → conformity-close).
...
Usage:
  ...
  python3 agent_level.py promote <agent>        # confer up to the earned tier
  python3 agent_level.py promote <agent> --to Advanced
  python3 agent_level.py promote --all           # confer every due agent (Wave-5 seed)
  python3 agent_level.py promote <agent> --to Intermediate --demote   # ratify a regression
...
def agent_by_id(guild, mid):
    for m in guild["agents"]:
        if m["id"] == mid:
            return m
    return None
...
def cmd_report(guild):
    agents = guild["agents"]
    due = [m for m in agents if m["levelInfo"]["dueForPromotion"]]
    over = [m for m in agents if m["levelInfo"]["overConferred"]]

    print("\n  AGENT LEVELS — Quartermaster's board")
    print("  " + "─" * 66)
    print(f"  {'agent':20}{'conferred':>14}{'earned':>14}{'AD':>5}  status")
    for m in sorted(agents, key=lambda m: (rank(m["levelInfo"]["earned"]), m["levelInfo"]["ad"]), reverse=True):
        li = m["levelInfo"]
    ...
    laggards = sorted(agents, key=lambda m: (rank(m["levelInfo"]["earned"]), m["levelInfo"]["ad"]))[:3]
    for m in laggards:
        li = m["levelInfo"]
        nxt = li["nextTier"]
        if not nxt:
...
def set_conferred(mid, tier):
    """Minimal-diff update of an agent's conferred `level` in agents-meta.json."""
    p = ROOT / "data/agents-meta.json"
    txt = p.read_text()
    pat = re.compile(r'("' + re.escape(mid) + r'":\s*\{.*?"level":\s*")[^"]*(")', re.DOTALL)
    new, n = pat.subn(lambda m: m.group(1) + tier + m.group(2), txt, count=1)
    if n != 1:
        sys.exit(f"could not locate conferred level for '{mid}' in agents-meta.json")
...
def log_promotion(mid, name, frm, to):
    ...
    r = subprocess.run([
        sys.executable, "tools/log_event.py",
        "--type", "agent-upgrade",
        "--who", "The Quartermaster",
        "--title", f"{direction} {name}: {frm} → {to}",
        "--detail", f"Conferred agent level {frm} → {to} (craft-depth tier). "
                    f"Prerequisite checklist for {to} verified against the build-derived "
                    f"signals per skillsmith/references/agent-leveling.md.",
        "--ref", mid,
...
def promote_one(guild, m, target, demote, assume_yes):
    ...
    if rank(target) > rank(earned):
        sys.exit(f"✗ refused: {mid} has only EARNED {earned}; cannot confer {target}. "
                 f"Deepen the arsenal first (see agent_level.py report).")
...
def cmd_promote(guild, who, target, do_all, demote, assume_yes):
    if do_all:
        due = [m for m in guild["agents"] if m["levelInfo"]["dueForPromotion"]]
        if not due:
        ...
        m = agent_by_id(guild, who)
        if not m:
            sys.exit(f"unknown agent '{who}'. Try one of: "
                     f"{', '.join(x['id'] for x in guild['agents'])}")
...
def main():
    ap = argparse.ArgumentParser(description="The Quartermaster's agent-leveling console.")
    sub = ap.add_subparsers(dest="cmd", required=True)
    sub.add_parser("report", help="promotion queue + laggard board")
    pp = sub.add_parser("promote", help="confer an agent's level")
    pp.add_argument("agent", nargs="?", help="agent id (omit with --all)")
    pp.add_argument("--to", dest="to", help="target tier (default: the earned tier)")
    pp.add_argument("--all", action="store_true", help="confer every due agent (seed)")
    ...
    if args.cmd == "report":
        ...
        if not args.all and not args.agent:
            sys.exit("name an agent or pass --all")
        cmd_promote(guild, args.agent, args.to, args.all, args.demote, args.yes)
