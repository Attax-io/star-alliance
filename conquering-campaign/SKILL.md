...
- **G0 — Campaign-worthiness triage-or-EXIT (the FIRST gate — runs before Step 0; the only gate that decides *whether* to run, not *how*).** Before creating ANY folder / frontmatter / `/goal` line / spawn: does the work clear the **BUILD bar** (≥3 real surfaces, OR genuinely multi-phase, OR must-stay-deployable staging across steps) or the **AUDIT bar** (a real doc↔code/DB reconciliation across the app)? If NOT → **EXIT**: state plainly "this is a {direct edit · single bug-fix · `/cleanup <mode>`}, not a campaign," route to the lighter tool, stop — no campaign artifacts. Route map: color / contrast / dark-mode → `/cleanup` (colors/lint) · i18n / translation → `/cleanup language` · postgres health / advisors → `/cleanup postgres` · lint / tsc noise → `/cleanup lint` · a single migration/view with no FE fan-out → direct edit under P3 show-SQL · a one-file / one-symbol bug → just fix it (`scope: bug-fix` ONLY for a shipped bug in a predecessor-excluded surface). **File count is NOT the bar — *surfaces / staging* is:** a 4-file change across view+type+2 components clears it; a 4-file color fix does not. `micro` is no longer a tier — it IS this exit. Override only on explicit user insistence ("run it as a campaign anyway"), recorded as `g0_override: <reason>`. Why this gate exists: the corpus showed 74% of campaign folders ≤3 files + `scope:` recorded in 0/116 plans — over-invocation was the one boundary never front-loaded into a gate, so the buried "offer a lighter pass" prose fired as a user complaint instead of a decision. (#101, #102)
...
# v3.7.0 over-invocation sprawl metric (G0 health) — run from project root; sums ALL campaign roots (monorepo has nested doc roots — §0.0.5, never `head -1`)
T=0; THIN=0; NOSCOPE=0
for R in $(find . -maxdepth 6 -type d -name '*-campaigns' 2>/dev/null); do for d in "$R"/*/; do [ -d "$d" ] || continue; T=$((T+1)); [ "$(ls "$d" | wc -l)" -le 3 ] && THIN=$((THIN+1)); P="$d/00-campaign-plan.md"; { [ -f "$P" ] && ! grep -q "^scope:" "$P"; } && NOSCOPE=$((NOSCOPE+1)); done; done
echo "campaign-folders=$T thin(≤3 files)=$THIN plans-missing-scope=$NOSCOPE  # baseline 2026-06-02 (lex root alone): 123 / 91 (74%) / 116 — G0 working ⇒ thin% + missing-scope% trend DOWN"
wc -l $SK/SKILL.md   # track core size — investigate if it climbs back toward 2000
```
A dangling `references/<file>` / `#N` / `§X`, or a core ballooning past ~1200 lines, is a self-health failure — fix in the same pass. A rising thin-folder% or no-scope-declaration% in the sprawl metric means G0 isn't firing (over-invocation regressing — #101/#102); the cure is enforcement at G0, not deleting folders after the fact.
...
## Reference files
- references/failure-modes.md — full catalog #1–#102 (prose + post-mortem)
