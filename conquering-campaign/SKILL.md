...
`C.X ?? '#hex'` fallback → drop it (tokens never undefined) · hardcoded hex w/o `theme-flat:` → token or comment · themable fg on themable bg (#52) → `alwaysWhite`/theme-flat · asymmetric-themable bg + fixed fg (#62) → bg-lock · chained Edits w/ lint-dirty intermediate (#28) → single `Write` · render-only CRUD verify (#35) → interaction recipe · mass-rename w/o forced_gate (#15) → add gate · pre-commit deletions (#14) → candidate + grep-proof · silent scope trim (#54) → declare tier · stale-cache symptom chase (#48) → restart dev server first · echo-back prescan as truth (#30) → `w0_useful: no` · inherited standing instr undeclared (#31) → `standing_instructions:` · hand-rolled primitive that exists (#27/#49) → G1 grep first · preview tools run when project owns preview (G4) → hand off recipe · new DB object w/o v2 conventions (#88) → G6 · hand-rolled inline RLS `EXISTS` → named bundle from RLS-BUNDLES.md (G6/W6) · `db().from().insert/update/delete` write → `callRpc` RPC (C4/G6) · `types.ts` full regen for a small change (#86) → hand-edit Row/Insert/Update · hand-rolled `fs.appendFileSync` batch loop on CLI (#93) → emitted `pipeline()`+`schema` workflow · Workflow tool as hard dependency / unprompted (#94) → §0.7 probe + EMIT-not-invoke · `git checkout -b` in a concurrent-actor tree (#95) → `git worktree add` · preview pipeline `MIGRATIONS_FAILED` (#96) → MCP `apply_migration` after show-SQL · irreversible DROP / money refactor under autonomous w/o show-gate (#97) → full-SQL gate + `RESTRICT` + delete-time re-grep · in-session audit lens treated as new campaign (#98) → §0.0.6 reopen + fold in place · `gh` absent (#99) → push + `pull/new` URL · audit "N files identical" count trusted in a BUILD (#100) → re-verify live, extract the provably-identical subset only · campaign folder/waves/spawn for ≤small / single-surface / `/cleanup`-mode / one-file-bug work (#101) → G0 EXIT + route to the lighter tool · a plan with no `scope:` declared (#102) → declare it to SIZE the run, but G0 (not scope) decides WHETHER to run · `scope: medium|full` on a 1–2-file folder (#103 scope-inflation) → it should have EXITed at G0; don't inflate the tier to pass the declaration gate · `USING(true)` on an `authenticated` multi-tenant table (#104) → `RLS-IMPERSONATE` proof + `pg_policies WHERE qual='true'` sweep · anon/SSR/sitemap reads a `security_invoker` view (#105, silent `42501`) → anon-granted `SECURITY DEFINER` RPC · a new gate/tier no live row satisfies (#106) → count the intended audience before "done" · `translateX(100%)`/`right:0` in an edge slide-over (#107) → `lib/rtl.ts` + `<SlideOverPanel>` + `useIsRtl`.
...
## Version history
Full history: **references/version-history.md**. Current: **v3.8.1 — 2026-06-04** (PATCH) — playbook patch from the latest sessions: **`RLS-IMPERSONATE`** named pattern + `pg_policies WHERE qual='true'` leak sweep (db-playbook, G3/G6, #104); G6 **anon-RPC public-read** sub-rule (anon base-SELECT → silent `42501`, #105); §5 **dormant-gate audience probe** (a gate no live row satisfies = silent no-op, #106); fe-i18n **RTL directional-transform** trap (`lib/rtl.ts`+`<SlideOverPanel>`, #107); hardened #96 (preview pipeline reliably dead → prod `BEGIN…ROLLBACK` dry-run IS the validation path). Prev: **v3.8.0 — 2026-06-04** (MINOR) — **made G0 bite**: the v3.7.0 gate did NOT reduce sprawl (measured 2 days post-ship — thin% flat 68% vs 71%, a 1-file `bug253` folder, 13 scope-inflated `medium`/`full`≤2-file folders), because G0 was still in-skill prose read only after the trigger matched. Fix = enforce *before* the decision: a **PreToolUse hook** (`.claude/scripts/g0-campaign-guard.sh`, matcher `Write`) surfaces a human G0 checkpoint on every NEW `00-campaign-plan.md` (resumes/edits pass; fail-open) + the **description trigger under-matches** (over-broad phrases qualified with the bar inline) + the **G0 bar = ≥3 INDEPENDENT logical changes** (a single vertical slice RPC+mutation+UI+i18n = ONE, not "4 surfaces") + the **self-health metric upgraded from presence to coherence** (scope-vs-reality mismatch + loose-file + off-enum-status, not just missing `scope:`) + **#103** (the recursive "a gate added to cure a buried rule is itself buried" meta-lesson). Mined in `references/REFLECTION-2026-06-04.md`. Prev: **v3.7.1 — 2026-06-02** (PATCH) — completed the lean-core-v2 extraction a parallel `cc WIP` commit started: **§Environment hazards → references/env-hazards.md** and **§Deploy + §Skill self-health → references/skill-maintenance.md** (SKILL.md keeps stub headings + pointers so every `§X` cross-ref still resolves); refreshed the extracted `skill-maintenance.md` to carry the v3.7.0 sprawl metric (the parallel copy was a pre-v3.7.0 snapshot, missing it). No behavior change — pure doc relocation. Prev: **v3.7.0 — 2026-06-02** (MINOR) — **Gate G0 campaign-worthiness triage-or-EXIT** (the FIRST gate — decides *whether* to run, not *how*; sub-campaign work ≤small / single-surface / a-`/cleanup`-mode / one-file-bug is declined + routed, no folder created) + **`micro` tier retired** into the G0 exit + **Trigger-precedence row 6b** (gates rows 7–9) + **§5.7/`/goal`/Workflow multiplier scope-gate** (emit/spawn only ≥ medium) + **§Skill-self-health sprawl metric** + **#101** (over-invocation / ceremony-inflation) + **#102** (scope-declaration ~68%, not enforced to 100%; and a declared scope SIZES a campaign but never asks whether to open one — sizing ≠ gating). Mined from "we used conquering too much lately" + the on-disk corpus: 161 folders, 71% ≤3 files, a color-fix campaign that spawned a color `/cleanup`. (Self-audit: an initial "0/116 declare scope" was a zsh `no-matches` glob-abort artifact — true rate 68%; corrected before ship, an #32 instance.) The skill's own meta-lesson ("a buried rule fires as a user correction") applied to its last un-gated boundary. Prev: **v3.6.2 — 2026-05-30** (PATCH) — from Campaign A (FE adopt-and-relocate): **#95 completed** with the worktree-has-no-`node_modules` verification gotcha (symlink from the primary checkout; the symlink resolves `@repo/*` to primary packages → confirm `packages/` clean) in §Env hazards; **#100** — a BUILD extending an AUDIT must re-verify its "N files share an identical block" count against live source before extracting (FE-2 held for only 4 of 10 rows / 8 of 15 fields; narrow to the provably-identical subset, never force consumer edits) in §Common failure modes + §Step 3/G1. Prev: **v3.6.1 — 2026-05-29** (PATCH) — from the app-wide-consolidation audit + Campaigns B/E: worktree-isolation vs `git checkout -b` under a concurrent actor (§Env hazards, #95); apply-via-MCP when the Supabase preview pipeline is `MIGRATIONS_FAILED` (references/db-playbook.md, #96); money/auth/irreversible-`DROP` full-SQL show-gate under autonomous + `RESTRICT` + delete-time re-grep (§Autonomous ladder + §5.1, #97); `gh`-absent PR handoff (§5.3 + §Env hazards, #99). Prev: **v3.6.0 — 2026-05-29** (MINOR) — §0.0.6 **audit same-session lens-extension** (the read-only AUDIT twin of §0.0.1 — reopen, `LN_lens` phase, fold into `99-synthesis.md` + the build-plan-proposal in place, edit the vault-log not duplicate) + a §0.0 resume row + a precedence-table row + #98. Prev: **v3.5.0 — 2026-05-29** — §0.7 Workflow-tool capability probe + skip table; §Step 2 **emits a ready-to-run Workflow script** (deterministic `pipeline`/`parallel`/`schema`/`resume`) per-wave after discovery, EMIT-not-invoke like `/goal`+`spawn_task`; `workflow-orchestration` named pattern; `workflow_available`/`workflow_emitted` frontmatter; #93–#94. Prev: **v3.4.0 — 2026-05-29** — §Subagent dispatch adds the **fan-out sweep** named pattern (sanctioned ≥5-disjoint-file parallel-subagent exception to W3-main-writes + main-agent structural-grep gate) + #91–#92. Prev: **v3.3.0 — 2026-05-29** — §5.7 spawns a self-contained follow-up **`/cleanup` session** (via `spawn_task`) at campaign close, handing its `99-risk-sweep.md` open-items to the cleanup skill's `followups` mode in a fresh session/worktree; durable fallback = the committed risk-sweep open-items; + #90. Prev: **v3.2.0** — §Step 2 emits a ready-to-paste **`/goal` line** at plan approval (transcript-only-evaluator-safe) + `goal_line:` + #89. **v3.1.0** — §Pre-flight Gate **G6** (DB-object conformity, W2–W6 doc-pointers) + G4 governance extension + #85–#88. **v3.0.0** — lean-core restructure + Pre-flight Gates + P8/P9/P10 + #64–#84. Bump `version:` on each meaningful upgrade; append the entry to references/version-history.md (newest first).
