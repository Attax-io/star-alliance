...
## Version history
Full release history (newest first) lives in **references/version-history.md**. Current: **v3.8.2 — 2026-06-15** (PATCH) — Cowork packaging: description trimmed to ≤1024 chars, inline §Version history recap + §Anti-pattern quick-lookup relocated to references/ (body 11.4k→10.3k words, no behavior change). On each meaningful upgrade, bump `version:` (SemVer) and prepend the entry to references/version-history.md.
