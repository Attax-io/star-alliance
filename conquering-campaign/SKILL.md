...
## Reference files
- references/failure-modes.md — full catalog #1–#89 (prose + post-mortem)
...
## Version history
Full history: **references/version-history.md**. Current: **v3.2.0 — 2026-05-29** — §Step 2 emits a ready-to-paste **`/goal` line** at plan approval (built from Q9 success criteria, phrased for `/goal`'s transcript-only evaluator) so one approval runs a campaign unattended under Claude Code's built-in goal-persistence Stop hook; `goal_line:` frontmatter + #89. Prev: **v3.1.0** — §Pre-flight Gate **G6** (DB-object conformity, W2–W6 doc-pointers) + G4 governance extension + #85–#88. **v3.0.0** — lean-core restructure + Pre-flight Gates + P8/P9/P10 + #64–#84. Bump `version:` on each meaningful upgrade; append the entry to references/version-history.md (newest first).
