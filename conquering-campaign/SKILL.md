...
## Reference files
- references/failure-modes.md — full catalog #1–#88 (prose + post-mortem)
...
## Version history
Full history: **references/version-history.md**. Current: **v3.1.0 — 2026-05-29** — §Pre-flight Gate **G6** (DB-object conformity: V2 naming + security boilerplate + RLS bundle catalog W6 + view-registry W3 + callRpc writes C4 + soft-delete-only C31, all as doc-pointers in db-playbook) + G4 governance extension (sacred `[!atta]` supremacy · docs-before-SQL · archived off-limits) + close-step project mechanics (submodule git · docs-daemon atomic write) + #85–#88, mined from the app's binding docs (W2–W6, V2-CONVENTIONS, RLS-BUNDLES, DESIGN-CANON) + project memory. Prev: **v3.0.0** — lean-core restructure + Pre-flight Gates + P8/P9/P10 + #64–#84. Bump `version:` on each meaningful upgrade; append the entry to references/version-history.md (newest first).
