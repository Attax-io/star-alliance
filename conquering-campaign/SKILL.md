...
The mine of 136 campaigns found the two most-frequent failures were ALREADY codified but buried — they fire as user corrections because the agent doesn't reach the rule at the decision moment. These five gates are short on purpose. Run the relevant one *before the action*, every time, regardless of scope or cadence.
...
| P4 | **Conformity over invention.** Read the pattern docs + the reference's full render function before writing. Inventing a new style is a failure. |
...
grep -oE "references/[a-z-]+\.md" $SK/SKILL.md | sort -u | while read f; do test -f "$SK/$f" || echo "MISSING $f"; done
...
## Reference files
- references/failure-modes.md — full catalog #1–#84 (prose + post-mortem)
- references/version-history.md — full release history
- references/db-playbook.md — PDAAV / PDAAV-RIBS + DB lessons + project schema facts
...
## Version history
Full history: **references/version-history.md**. Current: **v3.0.0 — 2026-05-29** — lean-core restructure + integrity repair (recreated the dangling `failure-modes.md` + `version-history.md`) + Pre-flight Gates + P8/P9/P10 + 21 new failure modes (#64–#84), from an exhaustive 136-campaign / 159-transcript mine. Bump `version:` on each meaningful upgrade; append the entry to references/version-history.md (newest first).
