...
description: >
  ...
  trigger bodies, view defs, RLS policy bodies, edge functions, function/RPC bodies,
  frontend types, frontend call sites, markdown docs, auto-memory, scheduled tasks,
  etc.) and the single most common failure mode is forgetting one. Load silently — no
  need to announce the skill is running.
metadata:
  version: 1.2.0
type: Skill
...
**If the rename touches a column, two surface classes the table+view sweep silently misses (conf 9/10, backed by 3 sessions):**

- **Function/RPC bodies (≠ trigger bodies / view defs).** A `RENAME COLUMN` does *not* rewrite the SQL text inside standalone functions or RPCs, and a `SECURITY DEFINER` function that still selects the old column keeps executing — the compiler never re-checks the body — so it breaks *silently* in prod. Rewrite every function/RPC body that names the old column in the same migration, using `ALTER FUNCTION`/`CREATE OR REPLACE FUNCTION`. Separately, watch for a **stale-named RPC surviving alongside the renamed one** (`get_task_*` left in place next to `get_td_*`) — the duplicate itself, not just its body, must be dropped.
- **Frontend call sites (≠ frontend types).** Regenerating `database.types.ts` fixes the *types*, not the hundreds of hand-written accesses — `row.task_name`, `.eq('task_id', …)`, `select('task_*')`, `.rpc('get_task_*')`, and any literal `'task_'` column/RPC strings. A column renamed `task_* → td_*` with the call sites left behind is what crashes the admin Tasks page. Grep `apps/web/` for the old column *and* RPC names and rewrite every hit in the same commit.
...
Then run the two extended-coverage scans — they catch the *silent* breakage a view/policy scan can't see:

- **Function/RPC body scan.** Scan `pg_get_functiondef` across `pg_proc` for the old name (e.g. `SELECT proname FROM pg_proc WHERE pg_get_functiondef(oid) ILIKE '%<old_name>%'`). Zero rows, or the `SECURITY DEFINER` function still selecting the old column ships silently. Also confirm no stale-named RPC survives next to its renamed twin.
- **Frontend call-site grep.** `rg` the old column and RPC names across `apps/web/` (accesses, `.eq(...)`, `select(...)`, `.rpc(...)`, and literal `'<old>'` strings). Zero hits, or the admin page crashes at runtime with the types looking green.
...
|---|---|---|
| **1.2.0** | 2026-07-12 | Extended scan coverage to two surface classes the table+view sweep silently missed (3 sessions, conf 9/10): **function/RPC bodies** (a `RENAME COLUMN` never rewrites standalone/`SECURITY DEFINER` function bodies — they break silently; plus stale-named RPCs surviving next to renamed twins) and **frontend call sites** (regenerating types fixes types, not hand-written `.task_name` / `.eq('task_id')` / `.rpc(...)` accesses — the `task_* → td_*` gap that crashed the admin Tasks page). Added Step 3 column-rename callouts and two Step 4 verification scans (`pg_proc`/`pg_get_functiondef` + `apps/web/` grep). Coverage-only — no change to the ALTER/batching procedure or trigger keywords. |
