...
# skillsmith — manage, sync, upgrade, create & auto-evolve Star Alliance skills (v1.1.10)
...
This skill is versioned (`metadata.version` in frontmatter, currently **1.1.10**) and self-registers in `VERSIONS.md`. Bump it on every change — PATCH for fixes/wording, MINOR for a new mode or capability, MAJOR for a breaking workflow change — and add a §Changelog row. (The `routine` mode upgrades skillsmith through exactly this contract — see §R6.)
