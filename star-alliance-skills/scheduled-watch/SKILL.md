...
2. **Pick the Claude model for each step of the tick.** The default worker is a cheaper Claude subagent (`haiku` or `sonnet`, spawned via the Task tool): it reads, scans, drafts, computes. Reserve a stronger Claude mind (`opus`, or `sonnet` for the gate) for the moment a side-effect is about to be committed (sending mail, posting to a calendar, mutating a Supabase row, filing a complaint). The gate reviews; the worker acts. Nothing ships unreviewed.
...
**Journeyman (ticks 10–100).** Stop trusting the clock — add jitter where overlapping ticks are possible. Move the gate from a per-tick review to a per-action review so the reviewing mind only sees what the worker is about to commit. Measure: *checkpoint write latency*, *replay-to-finish time from an injected failure*, *side-effects skipped because they were already applied*.
