...
**Swarm-worker model rule (the BUILD wave's parallel-write tier — see §W3 execution loop below + SKILL.md §Model assignment).** A swarm worker is a Claude subagent (spawned via the Task tool) that EDITS FILES, so it runs as the member's Claude model — Sonnet, per each member's `model:` field. Every worker is a Claude model; there is no separate engine to hand the work to. The saving is **N Sonnet subagents under one Opus orchestrator** (cheaper than Opus editing every slice serially). Per-slice: the work runs as the member's Claude model (Sonnet, or Opus per `model:`); coordinator + integration + per-slice/aggregate review = the Opus orchestrator (the live session). This reconciles with the table above: the same opus/sonnet tiering applies — opus orchestrates + reviews red/security slices, sonnet subagents run the disjoint mechanical/standard slices, Ollama prescans at W0. See [[decompose-and-swarm]] · [[weapon-utility]] · [[core-swarm]].
...
See [[decompose-and-swarm]] (the five MOVES in full) · [[core-swarm]] (the guild's swarm memory core) · [[weapon-utility]] (each worker runs as the member's Claude model).
