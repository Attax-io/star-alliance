...
- **RTL directional-transform trap (#107).** A hand-rolled physical transform in an edge slide-over — `transform: translateX(100%)`, `right: 0`, `left: 0` — does NOT flip under RTL: in Arabic the panel slides the wrong way and sticks mid-screen. Conformity canon (a G1 primitive): never hand-roll it — compose `lib/rtl.ts` helpers (`slideHiddenTransform` / `panelEdgeShadow` / `physicalSlideEdge`) + `useIsRtl` + the `<SlideOverPanel>` shell; LTR output stays byte-identical, RTL flips. The `EditorialFormShell` drawer fix covers 4 drawers transitively; nav drawers (width-collapse) + centered popups were already safe. A literal `translateX(100%)`/`right:0` in an edge panel is the grep smell. Deferred siblings: ~48 icons need `MIcon rtlFlip`, 8 toggles. Lex memory `[[discovery_rtl-slide-helper-canon]]`.
- **Bulk-select primitives are entity-id-type-agnostic (E6).** `useBulkEntityActions<Id extends string | number = number>` so a uuid-keyed entity (members) and an int-keyed one share the hook; `MemberListRow` gains `selectable` + a leading checkbox (entity-agnostic). Watch `checked` ≠ `selected` — panel-open state must not tick the box. Reinforces `[[discovery_primitive-collection-agnosticism]]` + `[[discovery_person-domain-bulk-delete-pattern]]`.
