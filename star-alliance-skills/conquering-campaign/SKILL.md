...
An AUDIT grows in-session when, near/after close, the user adds a NEW read-only lens ("also check access-correctness", "improve the audit"). NOT a new campaign, NOT a build pivot — precedence ~2 (alongside §0.0.1). Reopen the plan (`completed → in-progress`), append an `LN_lens` phase (`discovered_during: same-session-lens`), skip W0+W2, run a focused read-only W1/W3 for that lens, **fold it into the deliverables IN PLACE** (a `NN-<lens>.md` + an `## Extension lenses` block in `99-synthesis.md` + the build-plan-proposal), **edit the existing vault-log, never duplicate** (§5.3), re-close. Chaining lenses is fine (each its own `LN_lens` phase); a lens uncovering a SHIPPED bug or demanding a WRITE escalates OUT to a `bug-fix`/EXTENSION. (#98)
