...
| **W3 swarm worker — sonnet (BRAIN)** | member-instance (one per disjoint slice) | parallel disjoint WRITES; MiniMax (Doer seat) is the bulk doer INSIDE each | N×mid |
...
**Swarm workers run as the member BRAIN (Sonnet, tool-capable) — NEVER a doer-tier model** (MiniMax/Ollama can't hold Edit/Write/Bash); MiniMax is the doer INSIDE each worker. This reconciles with the table: opus orchestrates + reviews red/security, sonnet workers run disjoint mechanical/standard slices, Ollama prescans at W0. The saving is N Sonnet under one Opus, not a doer downgrade. See [[decompose-and-swarm]] · [[weapon-utility]].
