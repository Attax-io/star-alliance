---
name: python-master
description: "The Developer's craft for building production-grade Python libraries and web apps end to end — 14 skills from wdm0006/python-skills merged into one router. Covers project setup (pyproject/uv/ruff/pytest/pre-commit/CI), packaging + PyPI publishing, code quality (ruff/mypy, typing, idioms), testing (pytest fixtures, parametrization, mocking, Hypothesis), docs (docstrings, Sphinx, ReadTheDocs), API design + deprecation, CLI building (Click/Typer), performance (cProfile, memray, benchmarks), security audit (Bandit, pip-audit, Semgrep, detect-secrets), web-app architecture (FastAPI + async SQLAlchemy + Postgres + Stripe + Docker), git hygiene, release management (semver, changelogs), OSS community infra, and full library review. Use when starting/modernizing a Python library or service, configuring tooling, writing tests/docs, designing an API or CLI, profiling, auditing security, cutting a release, cleaning a repo, or reviewing a Python codebase. Triggers: 'set up a python library', 'add type hints', 'write pytest tests', 'package for pypi', 'profile this python', 'audit python security', 'design this api', 'build a python cli', 'cut a python release', 'review this python library', 'python web app architecture'."
license: MIT
metadata:
